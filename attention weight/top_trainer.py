...
class TopTrainer():
    def __init__(self,
        ...
        self.save_every_n_batches = 100 # 每100个batch保存一次
    ...
    # def save_attention(self, attention_info, batch_ids):
    #     """保存注意力到磁盘"""
    ...
    #         # 生成唯一标识
    #         uid = f"epoch{self.model_epoch}_batch{batch_id}_{time.strftime('%m%d%H%M')}"
            
    #         # 保存为压缩npz文件
    #         np.savez_compressed(
    #             os.path.join(self.attention_analyzer.save_dir, f"{uid}.npz"),
    #             attention=attn.numpy(),
    ...
    def run_batch(self, batch):
        ...
        if attention_info is not None:
            self.save_attention(attention_info, batch.batch.cpu().numpy())
